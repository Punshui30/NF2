...
export default function OnboardingSmart() {
  ...
  // Links + consent (stored only, no crawling here)
  ...
  return (
    <div className="min-h-screen bg-[#0b1026] text-white overflow-x-hidden">
          ...
          <div className="lg:col-span-2 space-y-5 sm:space-y-6">
            ...
            <section className="bg-white/5 border border-white/10 rounded-2xl p-4 sm:p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <VoiceInputButton
                  label="Speak vision"
                  onAppend={(t) => setLifeVision((v) => (v ? v + " " : "") + t)}
                />
              </div>
              ...
              <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <textarea
                    rows={3}
                    value={goal90d}
                    onChange={(e) => setGoal90d(e.target.value)}
                ...
                <div>
                  ...
                  <textarea
                    rows={3}
                    value={goal12m}
                    onChange={(e) => setGoal12m(e.target.value)}
              ...
              <div className="mt-3">
                ...
                <div className="text-xs text-white/60 mt-1">
                  {Math.min(lifeVision.length, MIN_VISION_LEN)} / {MIN_VISION_LEN} characters
                </div>
              </div>
            </section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      placeholder="Integrity, curiosity, freedom…"
                    ...
                    <VoiceInputButton
                      label="Speak"
                      onAppend={(t) =>
                        setValuesRaw((prev) => (prev ? prev + ", " : "") + t.replace(/\s+/g, " "))
                      }
                    />
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      value={antiValuesRaw}
                      onChange={(e) => setAntiValuesRaw(e.target.value)}
                      placeholder="Dishonesty, stagnation…"
                      className="flex-1 px-3 py-2 rounded-lg bg-white/10 border border-white/20 outline-none focus:ring-2 focus:ring-blue-500/40"
                    />
                    <VoiceInputButton
                      label="Speak"
                      onAppend={(t) =>
                        setAntiValuesRaw((prev) => (prev ? prev + ", " : "") + t.replace(/\s+/g, " "))
                      }
                    />
                  </div>
                </div>
            ...
            <section className="bg-white/5 border border-white/10 rounded-2xl p-4 sm:p-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    value={nonNegsRaw}
                    onChange={(e) => setNonNegsRaw(e.target.value)}
                    placeholder="Family time, honesty, health…"
                    className="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/20 outline-none focus:ring-2 focus:ring-blue-500/40"
                  />
                </div>
                <div>
                ...
                {defaultPatterns.map((p) => (
                  <div key={p.id} className="bg-white/5 border border-white/10 rounded-xl p-3">
                    ...
                    <input
                      ...
                      max={5}
                      step={1}
                      value={patterns[p.id] || 0}
                      onChange={(e) =>
                        setPatterns((prev) => ({ ...prev, [p.id]: parseInt(e.target.value, 10) }))
                      }
                      className="w-full"
                    />
                    <div className="text-xs text-white/60 mt-1">
            ...
            <div className="hidden sm:flex gap-3">
              <button
                className="px-4 py-2 rounded-xl bg-gradient-to-r from-blue-500 to-fuchsia-500 text-white font-semibold shadow transition hover:brightness-110"
                onClick={saveAndGo}
              >
            ...
            <section className="bg-white/5 border border-white/10 rounded-2xl p-4 sm:p-5">
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) => setLink("personal", e.target.value)}
                  className="w-full px-3 py-2 rounded-lg bg-white/10 border border-white/20 outline-none"
                />
              </div>
      ...
      {/* Mobile Sticky CTA */}
      <div
        className="fixed bottom-0 left-0 right-0 sm:hidden bg-[#0b1026]/95 border-t border-white/10 px-4 py-3"
        style={{ paddingBottom: "calc(env(safe-area-inset-bottom, 0px) + 12px)" }}
      >
        <button
          className="w-full py-3 rounded-xl bg-gradient-to-r from-blue-500 to-fuchsia-500 text-white font-semibold shadow"
          onClick={saveAndGo}
        >
          Continue to Dashboard
        </button>
