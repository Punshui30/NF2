...
// import { motion } from "framer-motion"; // DISABLE MOTION
import { ArrowRight, HelpCircle, Menu, X, Sparkles, Info } from "lucide-react";
// import HeroIntro from "../components/HeroIntro"; // DISABLE HERO INTRO initially too, to be safe.
import { useNavigate } from "react-router-dom";

/**
 * LandingPage (Motion-less Restore)
 * - Aurora background with contrast overlay
 * - Top nav with hidden admin tap (logo tapped 7x within 5s)
 * - Rich hero via <HeroIntro /> (Simulated here to avoid import crash)
...
export default function LandingPage({ onEnterApp, onLearnMore, onAdminOpen }: Props) {
  ...
  return (
    <div className="aurora-bg relative min-h-screen text-white border-4 border-blue-500">
      ...
      {/* Hero (INLINED & MOTION-LESS) */}
      <main className="relative z-10 flex min-h-[calc(100vh-80px)] items-center justify-center px-6">
        {/* Replaced HeroIntro with inline static content to verify */}
        <section className="mx-auto max-w-5xl px-4 md:px-8 text-center text-white">
          {/* Eyebrow */}
          <div className="mb-4 inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-xs font-medium tracking-wide">
            <Sparkles className="h-4 w-4" />
            Welcome to NorthForm
          </div>

          {/* Title */}
          <h1 className="text-4xl md:text-6xl font-semibold leading-[1.1] drop-shadow mb-4">
          ...
          {/* Subcopy */}
          <p className="mt-4 text-base md:text-lg text-white/95 leading-relaxed max-w-2xl mx-auto">
